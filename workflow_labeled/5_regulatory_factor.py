...
2. TF ranking metrics:
   ...
   - ProgramCouple from R mgcv GAM:
       ...
       edge weight, fit Gaussian GAMs for TF activity and target program score
       against pseudotime, predict both models on the same 100-point grid, then calculate:
...
Expected companion scripts:
    tf_driver/run_mgcv_tf_driver.R
    tf_driver/run_rra_tf_driver.R

Run:
    python workflow_unlabeled/6_regulatory_factor.py
"""
...
# User configuration
# ---------------------------------------------------------------------------
# This script can be run from workflow_unlabeled, while the R helper scripts
# live under the repository-level tf_driver directory.
SCRIPT_DIR = Path(__file__).resolve().parent
WORKFLOW_DIR = SCRIPT_DIR
R_HELPER_DIR = WORKFLOW_DIR.parent / "tf_driver"

# NETWORK_DIR = WORKFLOW_DIR / "results" / "pbmc_multiome_pymojitoo" / "network_analysis_CD4T"
# FULL_TRAJECTORY_INPUT_DIR = NETWORK_DIR / "full_trajectory_inputs"


# NETWORK_DIR = WORKFLOW_DIR / "7.7" / "network_analysis_CD4T_0.6"
# FULL_TRAJECTORY_INPUT_DIR = NETWORK_DIR / "full_trajectory_inputs"
NETWORK_DIR = WORKFLOW_DIR / "7.7" / "network_analysis_monocyte_0.6"
FULL_TRAJECTORY_INPUT_DIR = NETWORK_DIR / "full_trajectory_inputs"


# NETWORK_DIR = WORKFLOW_DIR / "results" / "pbmc_multiome" / "network_analysis_monocyte_archr"
# FULL_TRAJECTORY_INPUT_DIR = NETWORK_DIR / "full_trajectory_inputs"
...
def check_r_scripts() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            + "\n".join(missing)
            + "\nPlease place run_mgcv_tf_driver.R and run_rra_tf_driver.R "
              "under the repository-level tf_driver directory."
        )
...
def load_baseline_grn(input_dir: Path, manifest: dict) -> pd.DataFrame:
    ...
    if not grn_path.exists():
        raise FileNotFoundError(
            ...
            "to use a GRN from outside the selected input_dir."
        )
