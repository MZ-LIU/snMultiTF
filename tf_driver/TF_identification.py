...
# User configuration
# ---------------------------------------------------------------------------
# This script lives in the repo-level tf_driver folder so that the R helper
# scripts can stay next to it. The input data are generated by
# workflow_labeled/5.2_network.py under workflow_labeled/results.
SCRIPT_DIR = Path(__file__).resolve().parent
REPO_DIR = SCRIPT_DIR.parent
WORKFLOW_DIR = REPO_DIR / "workflow_labeled"

NETWORK_DIR = WORKFLOW_DIR / "results" / "pbmc_multiome" / "network_analysis_CD4T_0.5"
# Prefer the exact 5.2 SwitchTFI export folder if present; otherwise detect it.
SWITCHTFI_INPUT_DIR = (
    NETWORK_DIR / "switchTFI_inputs_naive_CD4_T_cells_to_memory_CD4_T_cells"
)
GRN_TABLE_PATH: Path | None = None

# NETWORK_DIR = WORKFLOW_DIR / "results" / "pbmc_multiome" / "network_analysis_monocyte"
# # Prefer the exact 5.2 SwitchTFI export folder if present; otherwise detect it.
# SWITCHTFI_INPUT_DIR = (
#     NETWORK_DIR / "switchTFI_inputs_classical_monocytes_to_intermediate_monocytes"
# )

OUTPUT_DIR = NETWORK_DIR / "tf_driver_ranking_7_2"
...
R_MGCV_SCRIPT = SCRIPT_DIR / "run_mgcv_tf_driver.R"
R_RRA_SCRIPT = SCRIPT_DIR / "run_rra_tf_driver.R"


# ---------------------------------------------------------------------------
# Small utilities
# ---------------------------------------------------------------------------
def find_switchtfi_input_dir(network_dir: Path) -> Path:
    if SWITCHTFI_INPUT_DIR.exists():
        return SWITCHTFI_INPUT_DIR

    manifests = sorted(
        network_dir.rglob("switchtfi_inputs_manifest.json"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    if not manifests:
        raise FileNotFoundError(
            f"No switchtfi_inputs_manifest.json found under {network_dir}. "
            "Please run workflow_labeled/5.2_network.py first."
        )
...
def main() -> None:
    ...
    input_dir = find_switchtfi_input_dir(NETWORK_DIR)
